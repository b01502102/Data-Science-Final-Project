...
clusterFileName = './%s/%s_%sData_unigram_cluster.txt' % (person,person,dataType)
...
for words in wordsSelected:
    ...
    l.write('    {"x": %f, "y": %f, "type": "%s", "size": %d}'% (term[0],term[1],words,size[words]))
...
count = 0
for term in model:
    l.write('    {"x": %f, "y": %f, "type": "%s", "size": %d}'% (term[0],term[1],str(count),10))
...
import os
htmlFileName = 'C:\Users\LENOVO\workspace\DataScience_Final\src\%s\%s_%sData_D3Scatter.html'% (person,person,dataType)
